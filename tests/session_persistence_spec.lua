...
describe('session persistence', function()
  ...
  describe('resume failure', function()
    it('removes session when resume exits with non-zero code', function()
      session = require('claude.session')
      ...
      local s = session.list()[1]
      -- Simulate CLI failing with exit code 1
      local result = session.on_exit(s.bufnr, 1)
    ...
    it('keeps session on normal exit (not a resume)', function()
      session = require('claude.session')
      local s = session.create('active')

      session.on_exit(s.bufnr, 1)

      assert.equals(1, session.count())
      assert.is_false(session.list()[1].is_alive)
    end)

    it('keeps session when resume exits with code 0', function()
      session = require('claude.session')
      session.create('good')
      session.save_state()
      ...
      local s = session.list()[1]
      local result = session.on_exit(s.bufnr, 0)
