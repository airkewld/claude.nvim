...
Neovim plugin for running multiple Claude Code sessions in floating terminal windows.
...
- **Delete past sessions** for the current project from a floating multi-select picker
...
Names persist through the CLI itself — the plugin stores nothing:

- Creating a named session (`:Claude new my-feature`) passes `--name my-feature` to the CLI, so the name survives restarts and shows up in the resume picker.
- Unnamed sessions get a name auto-generated by the CLI a few messages in.

Names live in the CLI, so the plugin no longer displays them in the floating window title.

## Window Title

The floating window title shows the current interaction mode:

```
 Claude - Interactive    ← typing in terminal mode
 Claude - Review         ← reading in normal mode
```
