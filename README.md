...
- Creating a named session (`:Claude new my-feature` or `n` in the menu) passes `--name my-feature` to the CLI, so the name survives restarts and shows up in the resume picker.
- Renaming (`r` in the sessions menu) forwards `/rename <name>` to the running session.
